...
impl Client {
    pub async fn execute<B: Business>(&self, business: B) -> Result<B::Output, ClientError> {
        business.execute_on(self).await
    }

    pub async fn execute_default<B: Business + Default>(&self) -> Result<B::Output, ClientError> {
